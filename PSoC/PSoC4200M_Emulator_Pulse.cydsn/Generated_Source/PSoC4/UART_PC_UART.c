...
#include "cyapicallbacks.h"
    ...
    * functionality when UART_PC_Sleep() function is called:
    * 0 – disable, other values – enable. Default value is 1.
    * It is only available when Enable wakeup from Deep Sleep Mode is enabled.
    */
    uint8 UART_PC_skipStart = 1u;
    /** \} globals */
    ...
    ***************************************/
    ...
    const UART_PC_UART_INIT_STRUCT UART_PC_configUart =
    {
        ...
        (uint8) UART_PC_UART_BYTE_MODE_ENABLE,
        (uint8) UART_PC_UART_CTS_ENABLE,
        (uint8) UART_PC_UART_CTS_POLARITY,
        (uint8) UART_PC_UART_RTS_POLARITY,
        (uint8) UART_PC_UART_RTS_FIFO_LEVEL
    };
    ...
    ****************************************************************************//**
    *
    *  Configures the UART_PC for UART operation.
    *
    *  This function is intended specifically to be used when the UART_PC
    *  configuration is set to “Unconfigured UART_PC” in the customizer.
    *  After initializing the UART_PC in UART mode using this function,
    *  the component can be enabled using the UART_PC_Start() or
    ...
    *******************************************************************************/
    void UART_PC_UartInit(const UART_PC_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            pinsConfig |= (0u != config->rtsRxFifoLevel) ? (UART_PC_UART_RTS_PIN_ENABLE) : (0u);
            pinsConfig |= (0u != config->enableCts)      ? (UART_PC_UART_CTS_PIN_ENABLE) : (0u);
        #endif /* !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1) */

            /* Configure pins */
            UART_PC_SetPins(UART_PC_SCB_MODE_UART, config->mode, pinsConfig);
            ...
            UART_PC_CTRL_REG     |= UART_PC_GET_CTRL_BYTE_MODE  (config->enableByteMode)      |
                                             UART_PC_GET_CTRL_ADDR_ACCEPT(config->multiprocAcceptAddr) |
                                             UART_PC_CTRL_UART;
        ...
        #if !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1)
            UART_PC_UART_FLOW_CTRL_REG = UART_PC_GET_UART_FLOW_CTRL_CTS_ENABLE(config->enableCts) | \
                                            UART_PC_GET_UART_FLOW_CTRL_CTS_POLARITY (config->ctsPolarity)  | \
                                            UART_PC_GET_UART_FLOW_CTRL_RTS_POLARITY (config->rtsPolarity)  | \
                                            UART_PC_GET_UART_FLOW_CTRL_TRIGGER_LEVEL(config->rtsRxFifoLevel);
        #endif /* !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1) */
            ...
            UART_PC_IntrTxMask = LO16(UART_PC_INTR_TX_MASK_REG);
    ...
    *******************************************************************************/
    ...
    #if !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1)
        UART_PC_UART_FLOW_CTRL_REG = UART_PC_UART_DEFAULT_FLOW_CTRL;
    #endif /* !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1) */
...
/*******************************************************************************
* Function Name: UART_PC_UartPostEnable
****************************************************************************//**
*
...
void UART_PC_UartPostEnable(void)
{
        ...
        if (UART_PC_CHECK_TX_SDA_MISO_PIN_USED)
        {
            /* Set SCB UART to drive the output pin */
            UART_PC_SET_HSIOM_SEL(UART_PC_TX_SDA_MISO_HSIOM_REG, UART_PC_TX_SDA_MISO_HSIOM_MASK,
                                           UART_PC_TX_SDA_MISO_HSIOM_POS, UART_PC_TX_SDA_MISO_HSIOM_SEL_UART);
        }
    #endif /* (UART_PC_TX_SDA_MISO_PIN_PIN) */

    #if !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1)
        #if (UART_PC_RTS_SS0_PIN)
            if (UART_PC_CHECK_RTS_SS0_PIN_USED)
            {
                /* Set SCB UART to drive the output pin */
                UART_PC_SET_HSIOM_SEL(UART_PC_RTS_SS0_HSIOM_REG, UART_PC_RTS_SS0_HSIOM_MASK,
                                               UART_PC_RTS_SS0_HSIOM_POS, UART_PC_RTS_SS0_HSIOM_SEL_UART);
            }
         ...
         /* Set SCB UART to drive the output pin */
        UART_PC_SET_HSIOM_SEL(UART_PC_TX_HSIOM_REG, UART_PC_TX_HSIOM_MASK,
                                       UART_PC_TX_HSIOM_POS, UART_PC_TX_HSIOM_SEL_UART);
    #endif /* (UART_PC_UART_TX_PIN) */

    #if (UART_PC_UART_RTS_PIN)
        /* Set SCB UART to drive the output pin */
        UART_PC_SET_HSIOM_SEL(UART_PC_RTS_HSIOM_REG, UART_PC_RTS_HSIOM_MASK,
                                       UART_PC_RTS_HSIOM_POS, UART_PC_RTS_HSIOM_SEL_UART);
    #endif /* (UART_PC_UART_RTS_PIN) */
#endif /* (UART_PC_SCB_MODE_UNCONFIG_CONST_CFG) */

    /* Restore TX interrupt sources. */
    UART_PC_SetTxInterruptMode(UART_PC_IntrTxMask);
}


/*******************************************************************************
* Function Name: UART_PC_UartStop
****************************************************************************//**
*
...
void UART_PC_UartStop(void)
{
        ...
        if (UART_PC_CHECK_TX_SDA_MISO_PIN_USED)
        {
            ...
            UART_PC_SET_HSIOM_SEL(UART_PC_TX_SDA_MISO_HSIOM_REG, UART_PC_TX_SDA_MISO_HSIOM_MASK,
                                           UART_PC_TX_SDA_MISO_HSIOM_POS, UART_PC_TX_SDA_MISO_HSIOM_SEL_GPIO);
        }
    #endif /* (UART_PC_TX_SDA_MISO_PIN_PIN) */
            ...
            if (UART_PC_CHECK_RTS_SS0_PIN_USED)
            {
                ...
                UART_PC_uart_rts_spi_ss0_Write(UART_PC_GET_UART_RTS_INACTIVE);

                /* Set GPIO to drive output pin */
                UART_PC_SET_HSIOM_SEL(UART_PC_RTS_SS0_HSIOM_REG, UART_PC_RTS_SS0_HSIOM_MASK,
                                               UART_PC_RTS_SS0_HSIOM_POS, UART_PC_RTS_SS0_HSIOM_SEL_GPIO);
            }
        #endif /* (UART_PC_RTS_SS0_PIN) */
    #endif /* !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1) */
        ...
        /* Set GPIO to drive output pin */
        UART_PC_SET_HSIOM_SEL(UART_PC_TX_HSIOM_REG, UART_PC_TX_HSIOM_MASK,
                                       UART_PC_TX_HSIOM_POS, UART_PC_TX_HSIOM_SEL_GPIO);
    #endif /* (UART_PC_UART_TX_PIN) */

    #if (UART_PC_UART_RTS_PIN)
        /* Set output pin state after block is disabled */
        UART_PC_rts_Write(UART_PC_GET_UART_RTS_INACTIVE);

        /* Set GPIO to drive output pin */
        UART_PC_SET_HSIOM_SEL(UART_PC_RTS_HSIOM_REG, UART_PC_RTS_HSIOM_MASK,
                                       UART_PC_RTS_HSIOM_POS, UART_PC_RTS_HSIOM_SEL_GPIO);
    #endif /* (UART_PC_UART_RTS_PIN) */
    ...
    UART_PC_UART_RX_CTRL_REG &= (uint32) ~UART_PC_UART_RX_CTRL_SKIP_START;
#endif /* (UART_PC_UART_WAKE_ENABLE_CONST) */

    /* Store TX interrupt sources (exclude level triggered). */
    UART_PC_IntrTxMask = LO16(UART_PC_GetTxInterruptMode() & UART_PC_INTR_UART_TX_RESTORE);
}
...
void UART_PC_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *   Check UART_PC_rxBufferOverflow to capture that error condition.
    *
    *******************************************************************************/
    ...
    *   - UART_PC_UART_RX_OVERFLOW - Attempt to write to a full
    *     receiver FIFO.
    *   - UART_PC_UART_RX_UNDERFLOW	Attempt to read from an empty
    *     receiver FIFO.
    *   - UART_PC_UART_RX_FRAME_ERROR - UART framing error detected.
    *   - UART_PC_UART_RX_PARITY_ERROR - UART parity error detected.
    *
    *  \sideeffect
    *   The errors bits may not correspond with reading characters due to
    *   RX FIFO and software buffer usage.
    ...
    *******************************************************************************/
    uint32 UART_PC_UartGetByte(void)
    {
    ...
    #if !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1)
        /*******************************************************************************
        * Function Name: UART_PC_UartSetRtsPolarity
        ****************************************************************************//**
        *
        ...
        *   - UART_PC_UART_RTS_ACTIVE_LOW  - RTS signal is active low.
        *   - UART_PC_UART_RTS_ACTIVE_HIGH - RTS signal is active high.
        *
        *******************************************************************************/
        void UART_PC_UartSetRtsPolarity(uint32 polarity)
        {
            if(0u != polarity)
            {
                UART_PC_UART_FLOW_CTRL_REG |= (uint32)  UART_PC_UART_FLOW_CTRL_RTS_POLARITY;
            }
            else
            {
                UART_PC_UART_FLOW_CTRL_REG &= (uint32) ~UART_PC_UART_FLOW_CTRL_RTS_POLARITY;
        ...
        /*******************************************************************************
        * Function Name: UART_PC_UartSetRtsFifoLevel
        ****************************************************************************//**
        *
        ...
        *******************************************************************************/
        void UART_PC_UartSetRtsFifoLevel(uint32 level)
        {
            ...
            uartFlowCtrl = UART_PC_UART_FLOW_CTRL_REG;

            uartFlowCtrl &= ((uint32) ~UART_PC_UART_FLOW_CTRL_TRIGGER_LEVEL_MASK); /* Clear level mask bits */
            uartFlowCtrl |= ((uint32) (UART_PC_UART_FLOW_CTRL_TRIGGER_LEVEL_MASK & level));

            UART_PC_UART_FLOW_CTRL_REG = uartFlowCtrl;
        }
    #endif /* !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1) */
    ...
    *******************************************************************************/
    void UART_PC_UartPutCRLF(uint32 txDataByte)
    {
    ...
    #if !(UART_PC_CY_SCBIP_V0 || UART_PC_CY_SCBIP_V1)
        /*******************************************************************************
        * Function Name: UART_PCSCB_UartEnableCts
        ****************************************************************************//**
        *
        ...
        *******************************************************************************/
        ...
        {
            UART_PC_UART_FLOW_CTRL_REG |= (uint32)  UART_PC_UART_FLOW_CTRL_CTS_ENABLE;
        }


        /*******************************************************************************
        * Function Name: UART_PC_UartDisableCts
        ****************************************************************************//**
        *
        ...
        *******************************************************************************/
        ...
        /*******************************************************************************
        * Function Name: UART_PC_UartSetCtsPolarity
        ****************************************************************************//**
        *
        ...
        *   - UART_PC_UART_CTS_ACTIVE_LOW  - CTS signal is active low.
        *   - UART_PC_UART_CTS_ACTIVE_HIGH - CTS signal is active high.
        *
        *******************************************************************************/
        void UART_PC_UartSetCtsPolarity(uint32 polarity)
        {
            if (0u != polarity)
            {
                UART_PC_UART_FLOW_CTRL_REG |= (uint32)  UART_PC_UART_FLOW_CTRL_CTS_POLARITY;
            }
            else
            {
                UART_PC_UART_FLOW_CTRL_REG &= (uint32) ~UART_PC_UART_FLOW_CTRL_CTS_POLARITY;
    ...
    *******************************************************************************/
    void UART_PC_UartSaveConfig(void)
    {
        ...
        */
        UART_PC_CLEAR_UART_RX_WAKE_INTR;
    ...
    *******************************************************************************/
    void UART_PC_UartRestoreConfig(void)
    {
        ...
        *******************************************************************************/
        ...
        {
        #ifdef UART_PC_UART_WAKEUP_ISR_ENTRY_CALLBACK
            UART_PC_UART_WAKEUP_ISR_EntryCallback();
        #endif /* UART_PC_UART_WAKEUP_ISR_ENTRY_CALLBACK */

            UART_PC_CLEAR_UART_RX_WAKE_INTR;

        #ifdef UART_PC_UART_WAKEUP_ISR_EXIT_CALLBACK
            UART_PC_UART_WAKEUP_ISR_ExitCallback();
        #endif /* UART_PC_UART_WAKEUP_ISR_EXIT_CALLBACK */
        }
    #endif /* (UART_PC_UART_RX_WAKEUP_IRQ) */
#endif /* (UART_PC_UART_RX_WAKEUP_IRQ) */
