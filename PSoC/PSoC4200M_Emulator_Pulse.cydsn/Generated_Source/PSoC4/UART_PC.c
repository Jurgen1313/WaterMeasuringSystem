    ...
    *******************************************************************************/
    void UART_PC_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        else if (UART_PC_SCB_MODE_SPI == mode)
        {
            ...
            hsiomSel[UART_PC_CTS_SCLK_PIN_INDEX] = UART_PC_CTS_SCLK_HSIOM_SEL_SPI;
            ...
            if (UART_PC_SPI_SLAVE == subMode)
            {
                ...
                pinsDm[UART_PC_CTS_SCLK_PIN_INDEX] = UART_PC_PIN_DM_DIG_HIZ;

            #if (UART_PC_RTS_SS0_PIN)
                /* Only SS0 is valid choice for Slave */
                hsiomSel[UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm  [UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_PIN_DM_DIG_HIZ;
            ...
            else 
            {
                ...
                pinsDm[UART_PC_CTS_SCLK_PIN_INDEX] = UART_PC_PIN_DM_STRONG;

            #if (UART_PC_RTS_SS0_PIN)
                hsiomSel [UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm   [UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_PIN_DM_STRONG;
                pinsInBuf |= UART_PC_RTS_SS0_PIN_MASK;
            #endif /* (UART_PC_RTS_SS0_PIN) */
            ...
            #if (UART_PC_CTS_SCLK_PIN)
                pinsInBuf |= UART_PC_CTS_SCLK_PIN_MASK;
            #endif /* (UART_PC_CTS_SCLK_PIN) */
            }
        }
        else /* UART */
        {
            ...
            else /* Standard or IrDA */
            {
                ...
                if (UART_PC_UART_MODE_STD == subMode)
                {
                    ...
                    #if (UART_PC_CTS_SCLK_PIN)
                        hsiomSel[UART_PC_CTS_SCLK_PIN_INDEX] = UART_PC_CTS_SCLK_HSIOM_SEL_UART;
                        pinsDm  [UART_PC_CTS_SCLK_PIN_INDEX] = UART_PC_PIN_DM_DIG_HIZ;
                    #endif /* (UART_PC_CTS_SCLK_PIN) */
                    }
                    ...
                    #if (UART_PC_RTS_SS0_PIN)
                        hsiomSel[UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_RTS_SS0_HSIOM_SEL_UART;
                        pinsDm  [UART_PC_RTS_SS0_PIN_INDEX] = UART_PC_PIN_DM_STRONG;
    ...
    #if (UART_PC_CTS_SCLK_PIN)
        UART_PC_SET_HSIOM_SEL(UART_PC_CTS_SCLK_HSIOM_REG,
                                       UART_PC_CTS_SCLK_HSIOM_MASK,
                                       UART_PC_CTS_SCLK_HSIOM_POS,
                                       hsiomSel[UART_PC_CTS_SCLK_PIN_INDEX]);

        UART_PC_uart_cts_spi_sclk_SetDriveMode((uint8) pinsDm[UART_PC_CTS_SCLK_PIN_INDEX]);

        UART_PC_SET_INP_DIS(UART_PC_uart_cts_spi_sclk_INP_DIS,
                                     UART_PC_uart_cts_spi_sclk_MASK,
                                     (0u != (pinsInBuf & UART_PC_CTS_SCLK_PIN_MASK)));
    #endif /* (UART_PC_CTS_SCLK_PIN) */

    #if (UART_PC_RTS_SS0_PIN)
        UART_PC_SET_HSIOM_SEL(UART_PC_RTS_SS0_HSIOM_REG,
                                       UART_PC_RTS_SS0_HSIOM_MASK,
                                       UART_PC_RTS_SS0_HSIOM_POS,
                                       hsiomSel[UART_PC_RTS_SS0_PIN_INDEX]);

        UART_PC_uart_rts_spi_ss0_SetDriveMode((uint8) pinsDm[UART_PC_RTS_SS0_PIN_INDEX]);

        UART_PC_SET_INP_DIS(UART_PC_uart_rts_spi_ss0_INP_DIS,
                                     UART_PC_uart_rts_spi_ss0_MASK,
                                     (0u != (pinsInBuf & UART_PC_RTS_SS0_PIN_MASK)));
    #endif /* (UART_PC_RTS_SS0_PIN) */
